...
def job_config = [
        job: [
                name: "project-10_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "feature-1670883555206"
        ]
]
...
def customLib = library identifier: 'project-10_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kobbyopoku/project-10_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.project-10.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "kobbyopoku", "project-10", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("project-10_Pipeline_develop");

job.with {

    authenticationToken('qL6Lel4URUm9zWLgmIRqHwHuCYwGBDyg')
